...
{
    // SYLAR_LOG_INFO(g_logger) << "fd: " << fd << " event: in" << event;
    RWMutexType::ReadLock lock(m_mutex);
    if ((int)m_fdContexts.size() <= fd) {
        return false;
    }
    ...
    --m_pendingEventCount;
    // SYLAR_LOG_INFO(g_logger) << "fd: " << fd << " event: out" << event;
    return true;
}
...
void IOManager::idle()
{
    ...
    while (true) {
        ...
        do {
            ...
            SYLAR_LOG_INFO(g_logger) << "epoll_wait error, errno=" << rt;
        } while (true);
